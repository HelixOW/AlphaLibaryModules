...
public class Util {
    ...
    public static <T> void cooldown(int length, final T key, final List<T> cooldownList) {
        ...
        }.runTaskLaterAsynchronously(ALMCore.getInstance(), length);
    }
    ...
    public static void runLater(long ticks, boolean async, Runnable timer) {
        if (async)
            ...
            }.runTaskLaterAsynchronously(ALMCore.getInstance(), ticks);
        else
            new BukkitRunnable() {
                public void run() {
                    timer.run();
                }
            }.runTaskLater(ALMCore.getInstance(), ticks);
    }
    ...
    public static void runTimer(long wait, long ticks, boolean async, Runnable timer) {
        if (async)
            ...
            }.runTaskTimerAsynchronously(ALMCore.getInstance(), wait, ticks);
        else
            new BukkitRunnable() {
                public void run() {
                    timer.run();
                }
            }.runTaskTimer(ALMCore.getInstance(), wait, ticks);
    }
